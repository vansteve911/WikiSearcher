...
def store_colleges_aliases(db_client, data_file):
  ...
  line_num = offset
  for line in lines:
    entity = parse_to_college_entity(line)
    if entity:
      res1 = add_college(db_client, entity)
      res2 = add_college_alias(db_client, entity)
      log_info = '[LINE-%s] load entity: %s, add college res: %s, add alias res: %s' % (line_num, entity, res1, res2)
      print(log_info)
    line_num += 1

def add_college(db_client, entity):
  attributes = json.dumps({
      'enWikiUrl': entity['en_wiki_url'],
      'zhWikiUrl': entity['zh_wiki_url'],
    })
  sql_tmpl = 'INSERT IGNORE INTO `college` (`id`, `code`, `en_name`, `zh_name`, `attributes`) VALUES (%s, %s, %s, %s, %s)'
...
def add_college_alias(db_client, entity):
  sql_tmpl = 'INSERT IGNORE INTO `college_alias` (`id`, `college_id`, `alias`) VALUES (%s, %s, %s)'
  row = (entity['alias_id'], entity['id'], entity['alias'])
  return db_client.execute(sql_tmpl, row, rollback_on_err = False)

def parse_to_college_entity(cols):
  ...
  if cols[0] == 'N/A':
    ...
    return
  return {
    'id': get_id('College', cols[0]),
    'alias_id': get_id('CollegeAlias', cols[1]),
    'code': cols[0],
    'alias': cols[1],
    'en_wiki_url': cols[2],
    'en_name': cols[3],
...
def main():
  ...
  data_file = '/Users/vansteve911/project/eliteEngine/data/listings/out_lowed.csv'
